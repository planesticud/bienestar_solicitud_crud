...
// @Param	body		body 	models.SolicitudServicio	true		"body for SolicitudServicio content"
// @Success 201 {int} models.SolicitudServicio
// @Failure 400 the request contains incorrect syntax
// @router / [post]
func (c *SolicitudServicioController) Post() {
	var v models.SolicitudServicio
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddSolicitudServicio(&v); err == nil {
...
// @Success 200 {object} models.SolicitudServicio
// @Failure 404 not found resource
// @router /:id [get]
func (c *SolicitudServicioController) GetOne() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v, err := models.GetSolicitudServicioById(id)
	if err != nil {
		c.Data["json"] = err.Error()
		logs.Error(err)
...
func (c *SolicitudServicioController) GetAll() {
	...
	l, err := models.GetAllSolicitudServicio(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = err.Error()
		logs.Error(err)
		//c.Data["development"] = map[string]interface{}{"Code": "404", "Body": err.Error(), "Type": "error"}
...
func (c *SolicitudServicioController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.SolicitudServicio{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateSolicitudServicioById(&v); err == nil {
			c.Data["json"] = "OK"
...
func (c *SolicitudServicioController) Delete() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	if err := models.DeleteSolicitudServicio(id); err == nil {
